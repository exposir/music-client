...
<div class="album-item-box" :class="classname">
    <div  v-if="fields.pic"  class="img-border">
        ...
        <div class="mask">
            <router-link :to="'/music/public/album/'+data.album_id" @click.native="commonFuns.crmStatistics('recommend','newAlbumPic')" ></router-link>
        </div>
        <div class="img-play" @click="playWholeAlbum(data.album_id)"></div>
    </div>
    <p v-if="fields.title" class="title-top" :title="data.title">
        <router-link :to="'/music/public/album/'+data.album_id" @click.native="commonFuns.crmStatistics('recommend','newAlbumName')">{{data.title}}</router-link>
    </p>
    <p v-if="fields.author" class="title-bottom" :title="data.author">
        <router-link  v-for="(item,i) in data.authorLinksArr"  :to="'/music/public/singer/'+item.artist_id+'/0'" @click.native="commonFuns.crmStatistics('recommend','newAlbumSinger')">{{item.artist_name}}<i v-if="i<data.authorLinksArr.length-1">,</i></router-link>
    </p>
    ...
    export default {
        ...
        methods: {
            playWholeAlbum(album_id) {
                this.commonFuns.crmStatistics('recommend','newAlbumPlay')
...
<style scoped>
    ...
    .albuminfo.album-item-box .img-border {
        height: 160px;
    }
